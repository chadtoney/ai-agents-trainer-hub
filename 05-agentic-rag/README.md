...
The Python Agent Framework notebook demonstrates production-ready RAG patterns with Azure AI Search:

- **Python: [Agent Framework with Azure AI Search](./code_samples/05-python-agent-framework.ipynb)**
  - File upload and vector store creation with Azure AI Agent Service
  - Hosted File Search Tool for RAG queries
  - Semantic search configuration for improved relevance
  - Azure AI Search integration with dynamic index creation
  - Complete document processing workflow

> **💡 Note for Students:** This notebook demonstrates enterprise-grade RAG patterns using:
> - **Azure AI Project**: Requires `AZURE_AI_PROJECT_ENDPOINT`, `AZURE_AI_MODEL_DEPLOYMENT_NAME`
> - **Azure AI Search**: Requires `AZURE_SEARCH_SERVICE_ENDPOINT`, `AZURE_SEARCH_API_KEY`, `AI_SEARCH_PROJECT_CONNECTION_ID`
> - **Authentication**: Uses Azure CLI Credential (`az login` required)
> 
> The notebook includes two examples:
> 1. **Simple RAG**: Using HostedFileSearchTool with vector stores
> 2. **Advanced RAG**: Direct Azure AI Search integration with semantic search and index management
>
> See [`.env.example`](../.env.example) at the repository root for all required configuration.
